...
export default {
  ...
  methods: {
    // BUG al salir de modificar archivo (botón cancelar) da error, y muestra mensaje de error
    elegirArchivo () {
      document.getElementById("fileArchivo").click();
    },
    async obtenerArchivos() {
      try {
        await this.$store.dispatch("getImagenes");
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error("image -> gallery -> fetch error -> " + error);
        this.$toast.open({
          message: "Hubo un error al intentar obtener los imagenes",
          type: "error",
          position: "top",
          duration: 5000,
        });
      }
      this.archivos = Helper.ordenarArrayCreadoDesc(this.$store.state.multimedia.imagenes)
    },
    cargarArchivo() {
    ...
    async editarDialogo(objeto) {
      try {
        ...
        this.$store.dispatch("modificarMultimedia", respuesta)
          .then((result) => {
