...
export default {
  ...
  methods: {
    ...
    async obtenerArchivos() {
      try {
        await this.$store.dispatch("getAudios");
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error("audio -> gallery -> fetch error -> " + error);
        this.$toast.open({
          message: "Hubo un error al intentar obtener los audios",
          type: "error",
          position: "top",
          duration: 5000,
        });
      }
      this.archivos = Helper.ordenarArrayCreadoDesc(this.$store.state.multimedia.audios)
    },
    cargarArchivo() {
